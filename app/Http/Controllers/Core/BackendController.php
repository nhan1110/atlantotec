...
class BackendController extends Controller
{
    ...
    public function _DestroyItem (){
        ...
        if(@$this->_DATA["_CTS_redirect"]){
            ...
            $data["_redirect"] = $this->_DATA["_BACKURL"];
        }
        $data["status"] = "success";
    ...
    public static function _GeneratorHTML($key,$item) {
        ...
        switch (@$item["type"]) {
            ...
            break;
            case 'file':
                $view_src = (@$item["value"]) ? '<a id="view-file" data-src="'.asset(@$item["value"]).'" href="javascript:;">View file</a>' : '';
                $html = '<div class="form-group m-form__group row"><div class="col-md-4"><label class="label" for="'.$key.'">'.@$item["label"].':</label></div><div class="col-md-8">
            ...
            case 'textarea':
                $html = '<div class="form-group m-form__group row"><div class="col-md-4"><label class="label" for="'.$key.'">'.@$item["label"].':</label></div><div class="col-md-8">
                    <div class="form-group">
                        <div class="form-line">
                            <textarea '.$attr.' rows="6" name="'.$key.'" class="not-editor form-control" '.(@$item["required"] == true ? 'required aria-required="true"' : '').' placeholder="Plase enter '.@$item["label"].'" >'.@$item["value"].'</textarea>
                            <div class="server-error none error" id="server-'.$key.'"></div>
                        </div>
                    </div>
                 </div></div>';
        ...
        foreach ($this->_ROLNAME as $key => $value) {
            foreach ($value as $key2 => $value2) {
                if (strpos($path,'/' .$value2 . '/') !== false || $lastPath == $value2) {
                    $action = $key;
                    $this->_MODULE = \App\Models\Rules::_validateRULE($role,trim($controller),$action);
                    break;
                }
            }
        }
